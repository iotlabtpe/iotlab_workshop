...
There are some components that invole in Amamzon FreeRTOS OTA updates:
...
* **OTA agent** runs on a device to publish requests to AWS IOT CORE and receives updates using HTTP or MQTT protocol when an update is available. It also checks the digital signature of downloaded files and install firmware updates.
...
We will go through the following steps to finish Amazon FreeRTOS OTA demostration in this lab:

1. Enable OTA agent and install initial version of firmware on ESP32
1. Prepare signed new firmware imageand create OTA updates using OTA updatemanager
...
3. Create an OTA update service role
   ...
   * Add IAM permisions to OTA service role
...
## Enable OTA agent and install initial version of firmware on ESP32 ##

1. OTA agent demostration configuration
...
3. Code-signing certificate configuration
   ...
   * Add self signed certificate for OTA agent
     * Open \<AmazonFreeRTOS>/demos/include/aws_ota_codesigner_certificate.h, and paste certificate to it. It should be formatted in following way, and please **replace YOUR_BASE64_CERTIFICATE_DATA with yours**
...
3. Create an OTA update
   ...
   * Under Job type, choose **Your job will complete after deploying to the selected devices/groups (snapshot)**, and enter an ID and description for your OTA update job, and then choose **Create**
